...
start_point = None
end_point = None
cached_route_type = None  # Cache the last selected route type
cached_heatmap = None     # Cache the last heatmap toggle state
...
def generate_routes():
    ...
    # Dynamically predict crime severity and update graphs
...
def find_routes(start_place=None, end_place=None, route_type=None, show_heatmap=None):
    ...
    safe_graph = adjust_weights_for_crime(G, crime_spots, radius=0.005)
    optimized_graph = adjust_weights_for_crime(G, crime_spots, radius=0.005)
    ...
    if show_heatmap:
        def add_crime_heatmap(crime_spots, date, time, day):
            """Add a heatmap layer to the map with dynamically predicted crime severity."""
            dynamic_crime_spots = [
                [lat, lon, predict_crime_severity(lat, lon, date, time, day)]
                for lat, lon in crime_spots
            ]
            HeatMap(dynamic_crime_spots, radius=15).add_to(pune_map)

        now = datetime.now()
        date = now.timestamp()
        time = now.hour
        day = now.weekday()
        add_crime_heatmap(crime_spots, date, time, day)
...
def update_map(route_type=None, show_heatmap=None):
    global generated_routes, start_point, end_point, cached_route_type, cached_heatmap
    ...
    # Check if the requested route type and heatmap state match the cached values
    if route_type == cached_route_type and show_heatmap == cached_heatmap:
    ...
    cached_route_type = route_type
    cached_heatmap = show_heatmap
    ...
    def add_route_to_map(route, graph, color, route_type, distance):
        ...
        # Create a popup with route details
        popup_content = (
            f"<b>{route_type} Route</b><br>"
            f"Distance: {distance / 1000:.2f} km<br>"
            f"Total Crime Severity: {total_severity:.2f}<br>"
            f"Crime Types: {', '.join(crime_types) if crime_types else 'None'}"
        )

        points = [(graph.nodes[node]['y'], graph.nodes[node]['x']) for node in route]
        folium.PolyLine(points, color=color, weight=5, popup=popup_content).add_to(pune_map)
    ...
    if route_type == 'all' or route_type == 'safest':
        if 'Safest' in generated_routes:
            add_route_to_map(
                generated_routes['Safest']['route'], G, 'green', 'Safest', generated_routes['Safest']['distance']
            )
    if route_type == 'all' or route_type == 'optimized':
        if 'Optimized' in generated_routes:
            add_route_to_map(
                generated_routes['Optimized']['route'], G, 'orange', 'Optimized', generated_routes['Optimized']['distance']
            )
...
def toggle_predicted_heatmap():
    ...
    if show_predicted_heatmap:
        def add_predicted_heatmap(crime_spots, date, time, day):
            """Add a heatmap layer to the map with dynamically predicted crime severity."""
            dynamic_crime_spots = [
                [lat, lon, predict_crime_severity(lat, lon, date, time, day)]
                for lat, lon in crime_spots
        ...
        time = now.hour
        day = now.weekday()
        add_predicted_heatmap(crime_spots, date, time, day)
