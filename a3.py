...
from route_calculations import get_astar_route, adjust_weights_for_crime, calculate_route_distance, crime_spots, G, update_graphs_with_dynamic_severity
...
def update_map(route_type=None, show_heatmap=None):
    ...
    if route_type == 'all' or route_type == 'safest':
        if 'Safest' in generated_routes:
            add_route_to_map(
                generated_routes['Safest']['route'], safe_graph, 'green', 'Safest', generated_routes['Safest']['distance']
            )
    if route_type == 'all' or route_type == 'optimized':
        if 'Optimized' in generated_routes:
            add_route_to_map(
                generated_routes['Optimized']['route'], optimized_graph, 'orange', 'Optimized', generated_routes['Optimized']['distance']
            )
